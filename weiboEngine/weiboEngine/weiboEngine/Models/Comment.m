...
@synthesize cellHeight,textBounds;
...
- (Comment*)initWithJsonDictionary:(NSDictionary*)dic {
	...
	if (self = [super init]) {
		...
		if (statusDic) {
			status = [[Status statusWithJsonDictionary:statusDic type:TWEET_TYPE_OTHER] retain];
		}
...
- (void)calcTextBounds {
    ...
    label.text = text;
    bounds = CGRectMake(10, 23, 300, 200);
    result = [label textRectForBounds:bounds limitedToNumberOfLines:20];
    textBounds = CGRectMake(bounds.origin.x, bounds.origin.y, 300, result.size.height);
    
    result.size.height += 33;
